...
def load_sessions_ensemble(data_dir, train_file, test_file):
    with open(os.path.join(data_dir, "data_desc.json"), "r") as fr:
        data_desc = json.load(fr)
    with open(os.path.join(data_dir, train_file), "rb") as fr:
        session_train = pickle.load(fr)
    with open(os.path.join(data_dir, test_file), "rb") as fr:
